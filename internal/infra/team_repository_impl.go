...
func (ri *TeamRepositoryImpl) Create(team *t.Team) (*t.Team, error) {
	...
	err = ri.DB.QueryRow("SELECT id, uuid, name, sport_id, grade_id, icon_path, description, address_state, address_city FROM teams WHERE id = ?", team.Id.Value()).Scan(&teamId, &lastInsertTeam.Uuid, &lastInsertTeam.Name, &sportId, &gradeId, &lastInsertTeam.IconPath, &lastInsertTeam.Description, &lastInsertTeam.AddressState, &lastInsertTeam.AddressCity)
	if err != nil {
